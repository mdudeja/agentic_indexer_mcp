...
export function registerGetBlastRadiusTool(server: McpServer) {
  server.registerTool(
    'get_blast_radius',
    {
      ...
      description: 'Find callers and dependents (files and symbols) that might break if you change a symbol.',
      inputSchema: z.object({
        symbol_name: z.string().describe('The exact name of the symbol being modified'),
      }),
    },
    async ({ symbol_name }) => {
      ...
      try {
        ...
        return { content: [{ type: 'text', text: `Error finding blast radius: ${err}` }], isError: true }
      }
    }
  )
}
