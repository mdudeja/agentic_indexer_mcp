  ...
  server.registerTool(
    'get_symbol_history',
    {
      ...
      inputSchema: z.object({
        symbol_id: z.string().optional().describe('The unique ID of the symbol (preferred)'),
        name: z.string().optional().describe('The name of the symbol (required if symbol_id is omitted)'),
        file_path_or_name: z.string().optional().describe('File path or partial file name (required if using name)'),
        include_diff: z.boolean().optional().describe('Whether to include the line diff for each commit (default: false)'),
      }),
    },
    async ({ symbol_id, name, file_path_or_name, include_diff }) => {
      ...
      try {
        ...
        if (symbol_id) {
          ...
          const files = await store.getFileByPartialNameOrPath(file_path_or_name)
          if (files.length === 0) {
            return {
              content: [{ type: 'text', text: `File not found matching: ${file_path_or_name}` }],
              isError: true,
            }
          }
          if (files.length > 1) {
          ...
          symbol = await store.getDefinitionByName(name as string, files[0]!.path)
        } else {
          throw new Error('Must provide either symbol_id, or both name and file_path_or_name')
        }
        ...
        if (symbol.end_line == null) {
          return {
            content: [{ type: 'text', text: `Symbol ${symbol.name} does not have line bounds in the database, cannot query git history.` }],
            isError: true,
          }
        }
        ...
        await updateUsage('get_symbol_history', [symbol.file_path], output.length)
        ...
        return {
          content: [{ type: 'text', text: `Error fetching symbol history: ${err}` }],
          isError: true,
        }
      }
    },
